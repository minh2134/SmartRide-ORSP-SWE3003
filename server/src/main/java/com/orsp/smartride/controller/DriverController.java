...
public class DriverController {
	...
	// Set driver as unready, but don't cancel ride
	...
	// Confirm pickup
	@MessageMapping("/driver/confirmpickup")
	@SendToUser("/topic/driver/response")
	Response confirmPickup(Principal principal, @Payload RideIDRequest request) throws Exception {
		String method = "/driver/confirmpickup";
		String username = principal.getName();
		...
		boolean success = driService.confirmPickup(username, request.getRideID());
		
		if (success) {
			return new Response(200, method);
		} else {
			ErrorResponse error = new ErrorResponse("Failed to confirm pickup");
			return new Response(400, method, error);
		}
	}
